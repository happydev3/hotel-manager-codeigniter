...
class sup_hotel_package_rates extends MY_Model {

    protected $_table_name = 'sup_hotel_package_rates';
    protected $_primary_key = 'sup_hotel_package_rates_id';
    protected $_primary_filter = 'intval';
    protected $_order_by = "sup_hotel_package_rates_id Desc";
    ...
    function get_single($id, $signal=TRUE) {
      ...
      function delete_room_rates($hotel_id, $room_id,$supplier_id,$package_rate_code, $startdate,$enddate) {
        $this->db->where('room_avail_date BETWEEN "' . $startdate . '" and "' . $enddate . '"');      
        $this->db->where('supplier_id', $supplier_id);      
        $this->db->where('room_id', $room_id);      
        $this->db->where('hotel_id', $hotel_id);
        $this->db->where('package_rate_code', $package_rate_code);      
