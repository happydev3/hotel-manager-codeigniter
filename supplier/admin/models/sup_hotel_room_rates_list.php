...
class sup_hotel_room_rates_list extends MY_Model {
    ...
    function getRoomRatesMealBased($array=NULL) {
        $this->db->select()->from($this->_table_name)->where($array);
        
        $this->db->group_by('meal_plan, from_date, to_date');
        $this->db->order_by($this->_order_by);
        $query = $this->db->get();
          if($query->num_rows > 0)
        {
     ...
     public function get_roomrates_duplicate($supplier_id, $sup_hotel_id,$sup_room_details_id) {
        $this->db->select('*');
...
function delete_room_rates_list_old($hotel_id, $room_id, $startdate,$enddate,$contract_id,$market,$meal_plan) {
   ...
   function delete_room_rates_type_list($rowarr)
    ...
    function delete_room_rates_list_new($rowarr)
    {
       $this->db->where('supplier_id',$rowarr['supplier_id']);
       $this->db->where('sup_hotel_id',$rowarr['sup_hotel_id']);   
       $this->db->where('hotel_code',$rowarr['hotel_code']);   
