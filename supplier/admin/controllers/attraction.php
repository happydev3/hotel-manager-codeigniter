...
function __construct() {
    ...
    $this->load->model('attraction_packages');
    // $this->load->model('attraction_theme');
    $this->load->model('attraction_itinerary');
    $this->load->model('holiday_city');
    // $this->load->model('attraction_accomodation');
    // $this->load->model('attraction_rates');
...
public function attraction_list() {
    $fields = 'id,supplier_id,attraction_type,attraction_name,attraction_code,city_covering,status';
    $data['packages'] = $packages = $this->attraction_packages->get_only_supplier($fields,$this->supplier_id);
    // echo '<pre>';print_r($data['itinerary']);exit;
    $data['sub_view'] = 'attraction/attraction_list';
    $this->load->view('_layout_main',$data);
}
public function get_active_packages(){
    $packages = $this->attraction_packages->get_active_packages();
    // echo '{"data":'.json_encode($packages).'}';
}
public function add_attraction() {
    // $data['theme'] = $this->attraction_theme->get_active_themes();
    $city_fields = 'city_id,city_name';
    $data['attraction_city'] = $this->holiday_city->get_active($city_fields);
    $data['currency'] = $this->currency->get('*');
    // $data['attraction_packages'] = $this->attraction_packages->get_active_supplier('id,attraction_name,status',$this->supplier_id);
    $this->load->model('closed_reason_metadata');
    $data['closedReasons'] = $this->closed_reason_metadata->get();
    $data['sub_view'] = 'attraction/add_attraction';
    $this->load->view('_layout_main',$data);
}
public function save_attraction_step1($check_insert='') {
    $this->form_validation->set_rules('attraction_type', 'attraction Type', 'trim|required');
    $this->form_validation->set_rules('attraction_name', 'attraction Name', 'trim|required');
    $this->form_validation->set_rules('attraction_code', 'attraction ID', 'trim|required');
    ...
    if($this->form_validation->run()==FALSE) {
        ...
        $data = array(
            ...
            'attraction_type' =>$this->input->post('attraction_type'),
            'attraction_name' =>$this->input->post('attraction_name'),
            'attraction_code' =>$this->input->post('attraction_code'),
        ...
        if($check_insert == ''){
            $insert_id = $this->attraction_packages->insert_attraction_packages($data);
            echo json_encode(array('insert_id' => $insert_id));
        } else{
            $insert_id = $check_insert;
            $this->attraction_packages->update($data, $insert_id);
            echo json_encode(array('insert_id' => $insert_id));
        }
...
public function quick_add() {
    $data['sub_view'] = 'attraction/quick_add';
    $this->load->view('_layout_main',$data);
}
public function set_package_status($id,$status) {
    ...
    $this->attraction_packages->set_status($data,$id);
    if($status == 0){
    ...
    redirect('attraction/attraction_list', 'refresh'); 
}

public function edit_attraction() {
 ...
 // echo '<pre>';print_r($data['theme']);exit;
    ...
    $fields = '*';
    $data['package_info'] = $package_info = $this->attraction_packages->get($fields,$package_id);
    $this->load->model('closed_reason_metadata');
    $data['closedReasons'] = $this->closed_reason_metadata->get();
    $data['sub_view'] = 'attraction/edit_attraction';
    $this->load->view('_layout_main',$data);
}
...
public function edit_step2() {
    $data['package_id'] = $package_id = $_GET['id'];
    $fields = 'overview,highlights,attraction_name,attraction_code';
    $data['package_info'] = $this->attraction_packages->get($fields,$package_id);

    $data['sub_view'] = 'attraction/edit_step2';
    $this->load->view('_layout_main',$data);
}
public function edit_step3() {
    $data['package_id'] = $package_id = $_GET['id'];
    $data['city_covering'] = $this->attraction_packages->get('city_covering',$package_id);
    $data['attraction_itinerary'] = $attraction_itinerary = $this->attraction_itinerary->get_itinerary('*',$package_id);
    $data['package_info'] = $this->attraction_packages->get('attraction_name,attraction_code',$package_id);
    if(!empty($attraction_itinerary)){
        $data['total_iti'] = count($attraction_itinerary);
    } else {
         $data['total_iti'] = 1;
    }
    // $data['pkg_accomodation'] = $this->attraction_packages->get_only_supplier('id,hotelaccomodation',$this->supplier_id,$package_id);
    // $data['accomodation_info'] = $this->attraction_accomodation->get_only_supplier('id,hotel_name',$this->supplier_id);
    // $data['gallery_img'] = $this->Upload_Model->get_images($package_id,'attraction_itinerary_images','*','package_id',$this->supplier_id);
    $data['attraction_city'] = $this->holiday_city->get_active('city_id,city_name');

    $data['sub_view'] = 'attraction/edit_step3';
    $this->load->view('_layout_main',$data);
}
public function edit_step4() {
    $data['package_id'] = $package_id = $_GET['id'];
    $fields = 'cancellation_policy,child_policy,insurance,includes,excludes,attraction_name,attraction_code';
    $data['package_info'] = $package_info = $this->attraction_packages->get($fields,$package_id);
    $data['attraction_packages'] = $this->attraction_packages->get_active_supplier('id,attraction_name,status',$this->supplier_id);
    // echo '<pre>';print_r($data['attraction_packages']);exit;
    $data['sub_view'] = 'attraction/edit_step4';
    $this->load->view('_layout_main',$data);
}
...
public function edit_step5() {
    ...
    $data['thumb_img'] = $this->attraction_packages->get('thumb_img',$package_id);
    $data['gallery_img'] = $this->Upload_Model->get_images($package_id,'attraction_package_images','*','package_id',$this->supplier_id);

    // $data['pkg_accomodation'] = $this->attraction_packages->get_only_supplier('id,hotelaccomodation',$this->supplier_id,$package_id);
    $data['package_info'] = $this->attraction_packages->get('attraction_name,attraction_code',$package_id);
    // $data['accomodation_info'] = $this->attraction_accomodation->get_only_supplier('id,hotel_name',$this->supplier_id);
    // echo '<pre>';print_r($data['pkg_accomodation']);exit;
    $data['sub_view'] = 'attraction/edit_step5';
    $this->load->view('_layout_main',$data);
}
...
public function update_all() { 
    ...
    if($step_no == 1){
        ...
        if($todo == 1){
            redirect('attraction/edit_step2?id='.$package_id, 'refresh');
        } else {
            redirect('attraction/edit_attraction?id='.$package_id, 'refresh');
        }
        
    } elseif ($step_no == 2) {
        $this->update_step2($package_id);
        if($todo == 1){
            redirect('attraction/edit_step3?id='.$package_id, 'refresh');
        } else {
            redirect('attraction/edit_step2?id='.$package_id, 'refresh');
        }
    } elseif ($step_no == 3) {
        ...
        if($todo == 1){
            redirect('attraction/edit_step4?id='.$package_id, 'refresh');
        } else {
            redirect('attraction/edit_step3?id='.$package_id, 'refresh');
        }
    } elseif ($step_no == 4) {
        ...
        if($todo == 1){
            redirect('attraction/edit_step5?id='.$package_id, 'refresh');
        } else {
            redirect('attraction/edit_step4?id='.$package_id, 'refresh');
        }
    }else{
...
public function update_step1($package_id){
    $this->form_validation->set_rules('attraction_type', 'attraction Type', 'trim|required');
    $this->form_validation->set_rules('attraction_name', 'attraction Name', 'trim|required');
    // $this->form_validation->set_rules('attraction_code', 'attraction ID', 'trim|required|is_unique[attraction_packages.attraction_code]');
    if($this->form_validation->run()==FALSE) {
        $data['sub_view'] = 'attraction/edit_attraction';
        ...
        $this->attraction_packages->update($data, $package_id);
...
public function update_step3($package_id){
    ...
    $this->attraction_itinerary->delete_itinerary($package_id);
    // $this->Upload_Model->delete_count_images($package_id,$day_count,'attraction_itinerary_images');
    // echo '<pre>';print_r($this->db->last_query());exit;
    for($a=0;$a<$day_count;$a++){
        ...
        $this->attraction_itinerary->insert($data);
...
public function add_theme() {
    $this->form_validation->set_rules('theme_name', 'Theme Name', 'trim|required|is_unique[attraction_theme.theme_name]');
    if($this->form_validation->run()==FALSE) {
        $data['theme_info'] = $this->attraction_theme->get_themes();
        $data['action'] = 'add_theme';
        $data['button'] = 'Add Theme';
        // Load common things here
        $data['sub_view'] = 'attraction/attraction_theme';
        ...
        $this->attraction_theme->insert_theme($data);
        $this->session->set_flashdata('message','Theme added successfully!');
        redirect('attraction/attraction_theme', 'refresh');
    }
}
...
public function edit_theme() {
    ...
    if($this->form_validation->run()==FALSE) {
        $data['theme_info'] = $this->attraction_theme->get_themes();
        $data['single_theme'] = $this->attraction_theme->get_single_theme($theme_id);
        ...
        $this->attraction_theme->update_theme($data, $theme_id);
        $this->session->set_flashdata('message','Theme updated successfully!');
        redirect('attraction/attraction_theme', 'refresh');
    }
}

public function attraction_theme() {
    // error_reporting(E_ALL);
    $data['theme_info'] = $this->attraction_theme->get_themes();
...
public function set_theme_status($id,$status) {
    ...
    $this->attraction_theme->set_theme_status($data,$id);
    if($status == 0){
        $msg = '<b style="color:#607d8b">Inactive</b>';
    } else {
        $msg = '<b style="color:#607d8b">Active</b>';
...
public function do_upload_old(){
    ...
    $day_count = $this->input->post('day_count');
    if(!empty($day_count)){
        // if($table_name=='attraction_itinerary_images'){
        //     $city_id = $this->input->post('city_id');
        //     $imgpath = 'uploads/supp_id/attraction/'.$city_id.'/'.$table_name.'/'.$day_count.'/'.$img_type.'/';
        //     $uploadpath = $this->upload_path.'supp_id/attraction/'.$city_id.'/'.$table_name.'/'.$day_count.'/'.$img_type.'/';
        // } else {
            $imgpath = 'uploads/attraction/'.$id.'/'.$table_name.'/'.$day_count.'/'.$img_type.'/';
            $uploadpath = $this->upload_path.'attraction/'.$id.'/'.$table_name.'/'.$day_count.'/'.$img_type.'/';
        // }
        
    } else {
        if($table_name=='attraction_packages' || $table_name=='attraction_package_images'){
            $tablepath = 'attraction_package_images';
        } else{
            $tablepath = $table_name;
        }
        $imgpath = 'uploads/attraction/'.$id.'/'.$tablepath.'/'.$img_type.'/';
        $uploadpath = $this->upload_path.'attraction/'.$id.'/'.$tablepath.'/'.$img_type.'/';
    }
...
public function do_upload(){
    ...
    $day_count = $this->input->post('day_count');
    if(!empty($day_count)){
            $imgpath = 'supplier/uploads/attraction/'.$id.'/'.$table_name.'/'.$day_count.'/'.$img_type.'/';
            $uploadpath = $this->upload_path.'attraction/'.$id.'/'.$table_name.'/'.$day_count.'/'.$img_type.'/';     
    } else {
        if($table_name=='attraction_packages' || $table_name=='attraction_package_images'){
            $tablepath = 'attraction_package_images';
        ...
        $imgpath = 'supplier/uploads/attraction/'.$id.'/'.$tablepath.'/'.$img_type.'/';
        $uploadpath = $this->upload_path.'attraction/'.$id.'/'.$tablepath.'/'.$img_type.'/';
    }
...
public function custom_upload($insert_id,$post,$to_do){
    ...
    // $imgpath = 'uploads/'.$supplier_id.'/attraction/'.$id.'/'.$tablepath.'/'.$img_type.'/';
    // $uploadpath = $this->upload_path.'attraction/'.$id.'/'.$tablepath.'/'.$img_type.'/';
    ...
    if($to_do == 'update'){
        ...
        redirect('attraction/add_accomodation?id='.$id, 'refresh');
    } else if($to_do == 'insert'){
        $this->session->set_flashdata('message','Added successfully!');
        redirect('attraction/add_accomodation', 'refresh');
    } else {
        // echo $to_do;exit;
    }
}
...
public function add_rates() {
    $data['package_id'] = $package_id = $_GET['id'];
    $data['rates_info'] = $this->attraction_packages->get('attraction_code,attraction_name,accomodation_type',$package_id);
    $data['currency'] = $this->currency->get('currency_id,currency_code');  

    $data['edit_rates_pack'] = $edit_rates_pack = $this->attraction_rates->get_rates_pack($package_id);
    if(empty($edit_rates_pack)){
        $data['sub_view'] = 'attraction/add_rates';
    } else {
        $data['edit_rates'] = $this->attraction_rates->get_rates($package_id);
        $data['economy_rates'] = $this->attraction_rates->get_economy_rates($package_id);
        $data['superior_rates'] = $this->attraction_rates->get_superior_rates($package_id);
        $data['first_class_rates'] = $this->attraction_rates->get_first_class_rates($package_id);
        $data['luxury_rates'] = $this->attraction_rates->get_luxury_rates($package_id);
        $data['sub_view'] = 'attraction/edit_rates';
    }
    $this->load->view('_layout_main',$data);
}
public function update_rates() {
    ...
    $this->attraction_rates->delete($package_id);
    $this->attraction_rates->delete_in_packages($package_id);
    ...
    if(!empty($economy)){
        ...
        $economy_attraction = $this->input->post('economy_attraction');
        ...
        for($r=0; $r<$counter;$r++){
            ...
            $this->attraction_rates->insert($economy_data);
        }
        $economy_data2 = array(
            'package_id' => $package_id,
            'transfer' => $economy_transfer,
            'attraction' => $economy_attraction,
            'hotel_rating' => $economy_hotel_rating,
            'accomodation_type' => $economy,
        );
        $this->attraction_rates->insert_in_packages($economy_data2);
    }
    ...
    if(!empty($superior)){
        ...
        $superior_attraction = $this->input->post('superior_attraction');
        ...
        for($s=0; $s<$counter;$s++){
            ...
            $this->attraction_rates->insert($superior_data);
        }
        
        $superior_data2 = array(
            'package_id' => $package_id,
            'transfer' => $superior_transfer,
            'attraction' => $superior_attraction,
            'hotel_rating' => $superior_hotel_rating,
            'accomodation_type' => $superior,
        );
        $this->attraction_rates->insert_in_packages($superior_data2);
    }
    ...
    if(!empty($first_class)){
        ...
        $first_class_attraction = $this->input->post('first_class_attraction');
        ...
        for($f=0; $f<$counter;$f++){
            ...
            $this->attraction_rates->insert($first_class_data);
        }
        
        $first_class_data2 = array(
            'package_id' => $package_id,
            'transfer' => $first_class_transfer,
            'attraction' => $first_class_attraction,
            'hotel_rating' => $first_class_hotel_rating,
            'accomodation_type' => $first_class,
        );
        $this->attraction_rates->insert_in_packages($first_class_data2);
    }
    ...
    if(!empty($luxury)){
        ...
        $luxury_attraction = $this->input->post('luxury_attraction');
        ...
        for($l=0; $l<$counter;$l++){
            ...
            $this->attraction_rates->insert($luxury_data);
        } 

        $luxury_data2 = array(
            'package_id' => $package_id,
            'transfer' => $luxury_transfer,
            'attraction' => $luxury_attraction,
            'hotel_rating' => $luxury_hotel_rating,
            'accomodation_type' => $luxury,
        );
        $this->attraction_rates->insert_in_packages($luxury_data2);
    }

    $this->session->set_flashdata('message','Rate Added successfully!');
    redirect('attraction/add_rates?id='.$package_id, 'refresh');
...
public function add_accomodation() {
    ...
    if(!empty($accomodation_id)){
        ...
        $data['edit_accomodation'] = $this->attraction_accomodation->get_single($accomodation_id);
    }
    $data['accomodation_info'] = $this->attraction_accomodation->get_only_supplier('*',$this->supplier_id);

    $data['gallery_img'] = $this->Upload_Model->get_images($accomodation_id,'accomodation_images','*','accomodation_id',$this->supplier_id);

    $data['attraction_city'] = $this->attraction_city->get_active('city_id,city_name');
    $data['sub_view'] = 'attraction/add_accomodation';
    $this->load->view('_layout_main',$data);
}
...
public function submit_accomodation() {
    ...
    if($this->form_validation->run()==FALSE) {
        ...
        if(!empty($accomodation_id)){
            $this->attraction_accomodation->update($data, $accomodation_id);
            $insert_id = $accomodation_id;
            $to_do = 'update';
        } else {
            $insert_id = $this->attraction_accomodation->insert($data);
            $to_do = 'insert';
        }
        ...
        if($add_meta == '1'){
            ...
            $result = $this->attraction_accomodation->get_only_supplier('id,hotel_name',$this->supplier_id);
...
public function set_acc_status($id,$status) {
    ...
    $this->attraction_accomodation->set_status($data,$id);
    if($status == 0){
        $msg = '<b style="color:#607d8b">Inactive</b>';
    } else {
        $msg = '<b style="color:#607d8b">Active</b>';
...
public function demo_add_routes() {
    $data['sub_view'] = 'attraction/route_demo';
    $this->load->view('_layout_main',$data);
}
public function demo_view_routes() {
    // echo '<pre>'; print_r($_POST);exit;
    $data['routes'] = $_POST;
    $data['sub_view'] = 'attraction/route_map_demo';
    $this->load->view('_layout_main',$data);
}
...
public function add_closed_rasons() {
    ...
    // $package_info = $this->attraction_packages->get('closed_reason',$package_id);
