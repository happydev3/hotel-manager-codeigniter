...
?>
        <?php
        $itinerary_destination = explode(',',$sightseeing_itinerary->itinerary_destination);
        // echo '<pre>'; print_r($sightseeing_itinerary);exit;
        ?>
        <div class="tab-content">
          <form action="<?php echo site_url() ?>sightseeing/update_all" method="post" class="step_form step3 upload-image-form2" steps="3" name="step3" method="POST" enctype="multipart/form-data" role="form" novalidate>
            ...
            <div class="tab-pane active" id="step-3">
              <div id="itinerary_wrapper" style="position: relative;">
                ...
                <?php if(!empty($sightseeing_itinerary)){ ?>
                <div id="itinerary_field_wrapper">
                  ...
                  for($i=0;$i<count($sightseeing_itinerary);$i++) {
                    $destination = explode(',',$sightseeing_itinerary[$i]->itinerary_destination);
                    $meals = explode(',',$sightseeing_itinerary[$i]->itinerary_meals);
                    $hotels = explode('|2|',$sightseeing_itinerary[$i]->hotelaccomodation);
                    $hotels2 = explode('|3|',$hotels[1]);
                    $acco1 = explode(',', $hotels[0]);
                    $acco2 = explode(',', $hotels2[0]);
                    $acco3 = explode(',', $hotels2[1]);
                    // $hotels3 = explode(',',$sightseeing_itinerary[$i]->hotelaccomodation3);
                    ...
                    <div class="boxs-body">
                      ...
                      <div class="row border_row">
                        <div class="add_images">
                          ...
                          <div class="form-group col-sm-12">
                            <a href="<?php echo site_url() ?>sightseeingdestination/city_images?city_id=<?php echo $desti ?>" class="<?php echo $desti ?>" target="_blank">Add Images for <?php $city_name = $this->holiday_city->get_iti_city($desti); echo $city_name[0]->city_name; ?></a>
                          </div>
                          <?php } } } ?>
                        </div>
                      </div>
                      ...
                      <div class="row border_row">
                        <div class="form-group col-sm-12">
                          ...
                          <textarea name="itinerary_description[]" id="itinerary_description<?php echo $i+1 ?>" class="form-control ckeditor" rows="3" cols="100"><?php echo $sightseeing_itinerary[$i]->itinerary_description ?></textarea>
                        </div>
                      </div>
                    </div>
                  </section>
...
<div class="modal fade" id="modalAccomodation" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      <form goingact="<?php echo site_url() ?>sightseeing/submit_accomodation" method="post" role="form" enctype="multipart/form-data" data-parsley-validate>
...
$(document).ready(function() {
  ...
  $('.desti_check').change(function() {
      ...
      if(this.checked) {
          var div = '<div class="form-group col-sm-12"><a href="'+site_url+'sightseeingdestination/city_images?city_id='+val+'" class="'+val+'" target="_blank">Add Images for '+text+'</a></div>';
...
$('.upload_now').on('click', function(){
    ...
    data.append('table_name', 'sightseeing_itinerary_images');
