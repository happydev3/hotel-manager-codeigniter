        ...
        <div class="pageheader">
          ...
          <div class="page-bar br-5">
            <ul class="page-breadcrumb">
              ...
              <li><a class="active">Add Activity</a></li>
            </ul>
          </div>
        </div>
      </div>
              ...
              <div class="row border_row">
                <div class="form-group col-md-4">
                  ...
                  <div class="check_icon">
                    <label class="checkbox-inline radio-custom2 radio-custom2-sm"><input type="radio" name="activity_type" value="Private" checked><i></i> Private</label>
                    <label class="checkbox-inline radio-custom2 radio-custom2-sm"><input type="radio" name="activity_type" value="Scheduled"><i></i> Scheduled <div class="backets_info">(SIC)</div></label>
                  </div>
                </div>
                <div class="form-group col-md-4">
                  <label class="strong" for="activity_name">Activity Name : </label>
                  <input name="activity_name" id="activity_name" type="text" class="form-control" autofocus="autofocus" required>
                </div>
                <div class="form-group col-md-4">
                  <label class="strong" for="activity_code">Activity Code : </label>
...
<div class="modal fade" id="modalClosedResons" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-sm">
    <div class="modal-content">
      <form goingact="<?php echo site_url() ?>activity/add_closed_rasons">
...
$(function() {
  ...
  //   var s_date = picker.startDate.format('YYYY-MM-DD');
  //   var e_date = picker.endDate.format('YYYY-MM-DD');
  //   // console.log(s_date);
  //   // console.log(e_date);
  //   if(s_date == e_date){
...
function save_activity(form, steps, todo){
    $ins_id = $("#insert_id").val();
    // alert(steps)
    $href = '<?php echo str_replace('supplier/', '', site_url()) ?>'+'activity/preview_activity/'+$ins_id;
    $('#previewid').attr('href', $href);

    $.ajax({
      type: 'post',
      url: '<?php echo site_url(); ?>activity/save_activity_'+steps+'/'+$ins_id,
      ...
      success: function(data) {
        ...
        $(".package_id").val(data.insert_id);
        if(todo == 1){
          window.location.replace('<?php echo site_url();?>activity/edit_step2?id='+data.insert_id);
        } else{
          window.location.replace('<?php echo site_url();?>activity/edit_activity?id='+data.insert_id);
        }
