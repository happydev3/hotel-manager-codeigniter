       ...
       <!--     <div class="row">
          ...
          <table class="table table-custom" id="advanced-usage">
            <thead>
             <tr>  
              ...
              <th>Availabel Rooms</th>
              <th>Fixed Price</th>
              <th>Extra bed Price(Adult)</th>
              <th>Extra bed Price(Child)</th>                  
             <!--  <th>Status</th>
              <th>Action</th> -->
              <th>Edit</th>                
            </tr>
          ...
          <tbody>
            <?php if(!empty($roomrates)) { 
              ...
              <tr>
                <td><?php echo $i+1; ?></td>
                <td><?php echo $roomrates[$i]->room_avail_date; ?></td>
                <td><?php echo $roomrates[$i]->rooms_available; ?></td>
                <td><?php echo $roomrates[$i]->room_fixed_rate; ?></td>
                <td><?php echo $roomrates[$i]->extra_bed_adult; ?></td>
                <td><?php echo $roomrates[$i]->extra_bed_child; ?></td>
                <td>
              ...
              <!--  <td> <?php  if($roomrates[$i]->status==1){ ?>           
                 <a class="btn btn-danger btn-xs"  onclick="return confirm('Do you really want to InActive this Room Rate. ?')"><i class="fa fa-times"></i> Inactive</a>
                 <?php } else { ?>
                 <a class="btn btn-success btn-xs"  onclick="return confirm('Do you really want to Active this Room Rate. ?')"><i class="fa fa-check"></i> Active</a> 
                 <?php }   ?>
               </td> -->
               <td><a class="btn btn-info btn-xs" href="<?php echo site_url(); ?>room/edit_room_rates/<?php echo $roomrates[$i]->sup_hotel_room_rates_id.'/'.$roomrates[$i]->room_code.'/'.$roomrates[$i]->hotel_code.'/'.$roomrates[$i]->supplier_id.'/'.$roomrates[$i]->sup_room_details_id.'/';?>"><i class="fa fa-pencil"></i> Edit</a></td>       
             </tr>  
             <?php } } ?>            
           </tbody>
         </table>
