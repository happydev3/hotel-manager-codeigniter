...
<div class="mainpanel">
  ...
  <div class="contentpanel">
    <!-- content goes here -->
    <h3>B2C Airline Discount Manager</h3>
    <ul class="nav nav-tabs nav-dark">
      <li class="active"><a href="#profile3" data-toggle="tab"><strong>B2C Airline Discount Manager</strong></a></li>
    </ul>
    <div class="tab-content mb30">
      <div class="tab-pane active" id="profile3">
        ...
        <table class='table' id="table5">
          ...
          <tbody>
            <?php if(!empty($b2c_discount_list)) {?>
            <?php
            for($i=0;$i<count($b2c_discount_list);$i++) {?>
            <?php ?>
            <tr>
              <td><?php echo $i+1;?></td>
              <td>
                <?php for($j=0;$j<count($airlines_list);$j++) {
                if($b2c_discount_list[$i]->airline==$airlines_list[$j]->airline_code){
                echo $airlines_list[$j]->airline_name;
                }
                } ?>
              </td>
              <td class="center"><?php if($b2c_discount_list[$i]->basefare==1){ echo 'Basefare,'; } if($b2c_discount_list[$i]->yqfare==2){ echo 'YQ Fare,'; }?></td>
              
              <td class="center"><?php
                if($b2c_discount_list[$i]->origin=='all'){
                echo 'all';
                }else{
                foreach($airport_list as $ar){
                if($ar->airport_code==$b2c_discount_list[$i]->origin){
                echo $ar->airport_name.','.$ar->airport_city.','.$ar->airport_country;
                }
                }
                }?>
              </td>
              <td class="center"><?php if($b2c_discount_list[$i]->destination=='all'){
                echo 'all';
                }else{
                foreach($airport_list as $ar){
                if($ar->airport_code==$b2c_discount_list[$i]->destination){
                echo $ar->airport_name.','.$ar->airport_city.','.$ar->airport_country;
                }
                }
              }?></td>
              <td class="center"><?php echo $b2c_discount_list[$i]->discount; ?></td>
              <td class="center">
                <?php if($b2c_discount_list[$i]->status == 0) { ?>
                <span class="label label-inactive">Inactive</span>
                <?php } else if($b2c_discount_list[$i]->status == 1) {?>
                <span class="label label-success">Active</span>
                <?php } ?>
              </td>
              <td class="center">
                <a class="" href="<?php echo site_url(); ?>/b2c/manage_discount_status/<?php echo $b2c_discount_list[$i]->markup_id; ?>/1" title="Active" onclick="return confirm('Do you want to activate this discount?')" data-rel="tooltip" >
                  <span class="glyphicon glyphicon-ok-sign"></span>
                </a>
                <a class="" href="<?php echo site_url(); ?>/b2c/manage_discount_status/<?php echo $b2c_discount_list[$i]->markup_id; ?>/0" title="Inactive" onclick="return confirm('Do you want to de-activate this discount?')" data-rel="tooltip" >
                  <span class="glyphicon glyphicon-minus-sign"></span>
                </a>
                <a class="" href="<?php echo site_url(); ?>/b2c/manage_discount_status/<?php echo $b2c_discount_list[$i]->markup_id; ?>/2" title="Delete" onclick="return confirm('Do you want to delete this discount?')" data-rel="tooltip" >
...
if(confirm('Are you sure you want to Add/Update B2C Specific Airline Discount?')) {
$.ajax({
url: '<?php echo site_url();?>/b2c/update_discount',
type: "POST",
data: dataString,
success: function (data) {
window.location = '<?php echo site_url();?>/b2c/flight_discount_manager';
}
});
}
});
