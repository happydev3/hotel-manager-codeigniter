				...
				<div class="span12">
					<div class="box">
						<div class="box-head tabs">
							<h3>Flights Cancellation Markup Manager</h3>
                           
                                
                           <ul class="nav nav-pills">                           
								<li class="active">
									<a data-toggle="tab" href="#hotel-markup">Flights Cancellation Markup Manager</a>
								</li>								
							</ul>							
						</div>
						<div class="box-content box-nomargin">
							<div class="tab-content">
									<div class="tab-pane active" id="hotel-markup">
                                    	<legend>Flights Cancellation Markup Master</legend> 
              ...
              <select id="selectError4" name="hotel_gen_country" required>
                ...
                <br/><br/><br/>
										...
										<table class='table table-striped dataTable table-bordered'>
											...
											<tbody>
                                           <?php if(!empty($canc_markup_list)) {?>
                          <?php $j=0;
						  for($i=0;$i<count($canc_markup_list);$i++) {?>
                             <?php if($canc_markup_list[$i]->service_type == 2 && $canc_markup_list[$i]->markup_type== 'generic') {?>
							<tr>
                                <td><?php echo $j+1;?></td>
                            	<td><?php echo $canc_markup_list[$i]->api_name;?></td>
								<td class="center"><?php echo $canc_markup_list[$i]->country;?></td>
								<td class="center"><?php echo $canc_markup_list[$i]->markup;?></td>             
                                <td class="center"><?php echo $canc_markup_list[$i]->updated_datetime;?></td>
								<td class="center">
                                <?php if($canc_markup_list[$i]->status == 0) { ?>
									<span class="label">Inactive</span>
                                 <?php } else if($canc_markup_list[$i]->status == 1) {?>
                                 <span class="label label-success">Active</span>
                                 <?php } ?>
								</td>
								<td class="center">
									
                                    <a class="btn btn-small manageflightCancellationStatus" href="javascript:void(0);" title="Active" data-rel="tooltip" data-base-url="<?php echo site_url();?>/" data-value="1" data-markup-id="<?php echo $canc_markup_list[$i]->markup_id;?>" >
										<i class="icon-ok"></i>			                                          
									</a>
                                     <a class="btn btn-small manageflightCancellationStatus" href="javascript:void(0);" title="Inactive" data-rel="tooltip" data-base-url="<?php echo site_url();?>/" data-value="0" data-markup-id="<?php echo $canc_markup_list[$i]->markup_id;?>" >
										<i class="icon icon-color icon-remove"></i>			                                          
									</a>
									<a class="btn btn-danger manageflightCancellationStatus" href="javascript:void(0);" title="Delete" data-rel="tooltip" data-base-url="<?php echo site_url();?>/" data-value="2" data-markup-id="<?php echo $canc_markup_list[$i]->markup_id;?>" >
...
$(document).ready(function() {
	...
	$("#hotel_generic").submit(function(ev){
		...
		if(confirm('Are you sure you want to Add/Update Cancellation MarkUp?')) {
			 
			 $.ajax({
				url: '<?php echo site_url();?>/home/update_canc_markups',
				type: "POST",
				data: dataString,
				
				success: function (data) {
					//alert(data);
					window.location = '<?php echo site_url();?>/home/canc_markup_manager_flights';
				}
