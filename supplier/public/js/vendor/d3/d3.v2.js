(function() {
  ...
  function d3_scale_quanboxs(domain, range) {
    function rescale() {
      var k = 0, n = domain.length, q = range.length;
      thresholds = [];
      while (++k < q) thresholds[k - 1] = d3.quanboxs(domain, k / q);
      return scale;
    }
    function scale(x) {
    ...
    scale.quanboxss = function() {
      return thresholds;
    };
    scale.copy = function() {
      return d3_scale_quanboxs(domain, range);
    };
    return rescale();
  }
  ...
  d3.median = function(array, f) {
    ...
    return array.length ? d3.quanboxs(array.sort(d3.ascending), .5) : undefined;
  };
  ...
  d3.scale.quanboxs = function() {
    return d3_scale_quanboxs([], []);
  };
