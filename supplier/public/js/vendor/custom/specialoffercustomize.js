...
function editrate(t){
    ...
    $offer_id=t.getAttribute('offer_id');
    $offer_type=t.getAttribute('offer_type');

    $("#ratecontent").html('');
  $.ajax({
    url: site_url+$val,
    data: {rateid:$rateid,room_code:$room_code,hotel_code:$hotel_code,room_id:$room_id,contract_id:$contract_id,offer_id:$offer_id,offer_type:$offer_type,hotel_id:$hotel_id},
    dataType: 'json',
    type: 'POST',
    success: function(data) {      
      if(data.edit_room_rates != '') {       
 ...
 function update_editrate(t)
  {
    ...
    { 
      ...
      if(parseFloat($("#discount_percentage").val())>=90)
      {
         alert("Value Should be Less than 90  for "+$("#discount_percentage").attr('placeholder'));
...
function add_rates(t)
 {
  ...
  /*   if($("#supplement_check:checked").length!=0)
   ...
   { 
     ...
     if(parseInt($("#min_no_of_stay_day").val())<=1)
      {
         alert("Value Should be Greater than 1  for "+$("#min_no_of_stay_day").attr('placeholder'));
             $("#min_no_of_stay_day").val('');
             $("#min_no_of_stay_day").focus();
             return false;
      }
       if(parseInt($("#max_no_of_stay_day").val())<=1)
      {
         alert("Value Should be Greater than 1  for "+$("#max_no_of_stay_day").attr('placeholder'));
             $("#max_no_of_stay_day").val('');
             $("#max_no_of_stay_day").focus();
             return false;
      }

       if(parseInt($("#max_no_of_stay_day").val())<parseInt($("#min_no_of_stay_day").val()))
      {
         alert($("#max_no_of_stay_day").attr('placeholder')+ " Should be  Greater than or equal to  "+$("#min_no_of_stay_day").attr('placeholder'));
             $("#max_no_of_stay_day").val('');
             $("#max_no_of_stay_day").focus();
             return false;
      }

       if(parseInt($("#no_of_stay_free_nights").val())==0)
      {
         alert("Value Should be Greater than 0  for "+$("#no_of_stay_free_nights").attr('placeholder'));
             $("#no_of_stay_free_nights").val('');
             $("#no_of_stay_free_nights").focus();
             return false;
      }

       if(parseInt($("#no_of_stay_free_nights").val())>=parseInt($("#min_no_of_stay_day").val()))
      {
         alert($("#no_of_stay_free_nights").attr('placeholder')+ " Should be  less than "+$("#min_no_of_stay_day").attr('placeholder'));
              $("#no_of_stay_free_nights").val('');
             $("#no_of_stay_free_nights").focus();
             return false;
      }
