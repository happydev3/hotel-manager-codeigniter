...
<div class="wraper container-fluid">
	<div class="page-title">
		<h3 class="title">Add/Update Country</h3>
	</div>
	<div class="row">
		<div class="col-md-12">
					...
					<div class="col-lg-12">
						<div id="collapseOne-2" class="panel-collapse">
							<div class="panel-body">
								<form action="{{url('tours/regions/updateCountry')}}" method="post">
									...
									<div>
										<section>
											<div class="row form-group">
												...
												<div class="col-md-3">
													<select name="continent_name" class="form-control" tabindex="-1" required="">
														<optgroup label="Continent List">
															@foreach($continent_list as $val)
															<option value="{{$val->continent_id}}" @if(DB::table('holiday_country')->where('country_id', $country_id)->value('continent_id')==$val->continent_id) selected @endif>{{$val->continent_name}}</option>
															@endforeach
														</optgroup>
													</select>
													@if ($errors->has('continent_name'))
													<span class="help-block text-danger">
														<strong>{{ $errors->first('continent_name') }}</strong>
													</span>
													@endif
												</div>
											</div>
											<div class="row form-group">
												...
												<div class="col-md-3">
													<input type="text" name="country_name" value="{{DB::table('holiday_country')->where('country_id', $country_id)->value('country_name')}}" class="form-control" required="">
				...
				<div class="panel-body">
					<table id="datatable-buttons" class="table table-striped table-bordered">
						...
						<tbody>
							...
							<tr>
								<td>{{$key+1}}</td>
								<td>{{DB::table('holiday_continent')->where('continent_id', $val->continent_id)->value('continent_name')}}</td>
								<td>{{$val->country_name}}</td>
								<td>
									<a href="{{url('tours/regions/country/'.$val->country_id)}}" class="label label-info" title="Edit"><i class="fa fa-edit"></i></a>
								</td>
							</tr>
							@endforeach
						</tbody>
