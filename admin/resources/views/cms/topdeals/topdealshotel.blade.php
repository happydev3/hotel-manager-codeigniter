...
<div class="wraper container-fluid">
	<div class="page-title"> 
		<h3 class="title">Top Deals Hotel @if($topdeals!=''){{ ' For '.$topdeals}}@endif</h3> 
	</div>
	<div class="row">
		<div class="col-md-12">
			<div class="panel panel-default">
				<div class="panel-heading">
					<h3 class="panel-title"><a href="{{url('cms/topdeals')}}" class="label label-default" title="Add New" style="color: #fff"><i class="fa fa-plus"></i>Go To Top Deals </a></h3>
				</div>
				@if(session('success'))
				...
				<div class="panel-body">
					 <form action="{{ url('cms/topdealshotel')}}" method="post" enctype="multipart/form-data" data-parsley-validate>
					 	{{ csrf_field() }}
						<div>							
							<section>
								<div class="row"> 
									<div class="col-md-10"> 
										<div class="form-group"> 
											<label for="title" class="control-label">Hotel Name</label>
											 <select  id="topdealshotellist" name="name" class="form-control" tabindex="-1" required="required"></select> 
				...
				<div class="panel-body">
					<table id="datatable-buttons" class="table table-striped table-bordered">
						...
						<tbody>
							@forelse($topdealshotel as $key=>$val)
							<tr>
								 <td>{{$key+1}}</td>
								 <td>{{$val->api}}</td>
								 ...
								 <td>
									@if($val->status==0)
									<a href="{{ url('cms/topdealshotel/status/'.$val->id.'/1')}}" class="label label-success" title="Active"><i class="fa fa-check"></i></a>
									@elseif($val->status==1)
									<a href="{{ url('cms/topdealshotel/status/'.$val->id.'/0')}}" class="label label-danger" title="In active"><i class="fa fa-remove"></i></a>
									@endif
								 </td>
							</tr>
							@empty
...
<script src="https://cdnjs.cloudflare.com/ajax/libs/parsley.js/2.8.0/parsley.js"></script>
 <script src="https://cdnjs.cloudflare.com/ajax/libs/select2/4.0.5/js/select2.js"></script>
  ...
  $('#topdealshotellist').select2({
    minimumInputLength: 2,
    minimumResultsForSearch: 10,
    ajax: {
        url: "{{ url('topdealshotel/hotellist/'.$code)}}",
        dataType: "json",
        type: "GET",
