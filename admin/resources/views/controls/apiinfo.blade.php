					...
					<table id="datatable-buttons" class="table table-striped table-bordered">
						...
						<tbody>
							...
							<tr>
								...
								<td>{{$val->api_name}}</td>
								<td>
									@if($val->service_type==1) Hotel @elseif($val->mode==2) Flight @endif
								</td>
								...
								<td>
									@if($val->status==0)
									<a href="{{ url('b2b/hotels/apiinfo/status/').'/'.$val->id.'/1'}}" class="label label-success"  title="re-activate" onclick="return confirm('You are about to re-activate this api');"><i class="fa fa-check"></i></a>
									@elseif($val->status==1)
									<a href="{{ url('b2b/hotels/apiinfo/status/').'/'.$val->id.'/0'}}" class="label label-warning" title="de-activate" onclick="return confirm('You are about to de-activate this api');"><i class="fa fa-remove"></i></a>
									@endif
