				...
				<div class="panel-body">
					<table id="datatable-buttons" class="table table-striped table-bordered">
						...
						<tbody>
							...
							<tr>
								<td>{{$key+1}}</td>
								<td>{{$users_info->user_no}}</td>
								<td>{{$users_info->first_name}}</td>
								<td>{{$users_info->user_email}}</td>
								...
								<td>
									...
									<a href="{{ url('b2c/users/status/'.$users_info->id.'/1')}}" class="label label-success" title="Active"><i class="fa fa-check"></i> Re-active</a>
									@elseif($users_info->status==1)
									<a href="{{ url('b2c/users/status/'.$users_info->id.'/0')}}" class="label label-danger" title="In active"><i class="fa fa-remove"></i> In-active</a>
									@endif
									<div class="clearfix" style="margin-top: 5px;"></div>
									<a href="{{url('b2c/users/'.$users_info->id.'/edit')}}" class="label label-info" title="Edit"><i class="fa fa-edit"></i> Edit</a>
