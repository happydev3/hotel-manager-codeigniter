...
<script type="text/javascript">
  $(document).ready(function(){
    $('#subscribe-opt').submit(function(e){
      e.preventDefault();
      var _this = $(this);
      var datasend = _this.serialize();
      // console.log(datasend);
      $.ajax({
        url: '<?php echo site_url();?>home/subscribeAjax',
        data: datasend,
        dataType: 'json',
        type: 'POST',
        beforeSend: function(){
        ...
        success: function(data){
          // console.log(data);
          // console.log(_this.find('.member_signup').val());
          if(data.status == '1'){
            if(_this.find('.member_signup').val() == 'Sign Up, It\'s Free!'){
              $('#modalRegister').modal('show');
              $('#acc_user_email').val(_this.find('.email-holder').val());
              $('#acc_user_password').val('');
            } else {
              $('#subs-row').hide();
            }
          } else{
