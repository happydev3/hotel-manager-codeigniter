...
<style type="text/css">
  .accordions-content p,.hotel-dtls-amenities ul li {
    color: #616161;
  }
</style>
      ...
      <div class="row no-padding">
        ...
        <div class="col-md-5">
          <div class="price-details">
            <a id="jump_rooms2" href="#htl-rooms-book" class="btn book-btn">Book Now <span class="fa fa-caret-down"></span></a>
          </div>
          <div class="price-details">
            <h2 class="price-tag">
              <small>from </small>
              ...
              <div id="reservation-form" class="book-tours">
                ...
                <?php } ?>
                  ...
                  <div class="row2">
                    ...
                    <div class="col-sm-12">
                      <?php if($villaDetails->availability_type==1) { ?>
                        <button class="btn book-btn btn-block">Book now</button>
                      <?php } else { ?>
                        <button class="btn book-btn btn-block">Request a booking</button>
                      <?php } ?>
                    </div>
                  </div>
                </form>
                <div class="row2">
                  <div class="col-sm-12">
                    <hr>
                    <a class="btn btn-default btn-block" data-toggle="modal" data-target="#enquiry_request">Ask a question</a>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div class="white-box accordions-content nearby grid-view" id="nearby-property">
            ...
            <div class="result-grid hotel-grid result-box nearby-box">
              <div class="row left-section">
                <div class="col-sm-12">
                  <div class="htl-img">
                    <img src="<?php echo getNearbyHotelImage($similar_data[$s]->thumb_img) ?>" alt="<?php echo $similar_data[$s]->property_name ?>" class="img-responsive" />
                  </div>
                </div>
              </div>
              ...
              <div class="row no-padding right-section">
                <div class="col-sm-12">
                  ...
                  <a href="<?php echo site_url().'villa/details/'.base64_encode($similar_data[$s]->session_id.'/'.$similar_data[$s]->uniqueRefNo.'/'.$similar_data[$s]->search_id.'/'.$similar_data[$s]->villa_code.'/'. base64_encode('villa_crs')); ?>" class="btn blue-btn form-control radius-two" style="font-size: 18px;font-weight: bold;">Show Prices</a>
                </div>
              </div>
            </div>
            <?php } } else { ?>
        ...
        <form action="" method="POST">
          ...
          <div class="row">
            <div class="col-sm-2 text-center">
              <input type="submit" name="villa_inquiry" value="Send Inquiry" class="btn btn-primary" style="background:#ff9537;">
            </div>
          </div>
        </form>
      </div>
...
<div class="modal fade" id="enquiry_msg" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-md">
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
        <h3 class="modal-title" style="visibility: hidden;">Ask a question</h3>
      </div>
      <div class="modal-body">
        <p>Thank you for submitting a question. We will contact you with a response as soon as possible.</p>
      </div>
    </div>
  </div>
</div>
...
<?php 
$inquiry = $this->session->flashdata('inquiry');
if($inquiry=='yes'){ ?>
<script type="text/javascript">
  $('#enquiry_msg').modal('show'); 
</script>
<?php } ?>
...
<script>
  var images = [
    <?php for($g=0;$g<count($galleryimg);$g++) { ?>
      '<?php echo getGalleryImage($galleryimg[$g]->gallery_img) ?>',
    <?php }  ?>
  ];
  // console.log(images);
